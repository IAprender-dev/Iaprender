...
import { users, gestores, diretores, professores, alunos } from '@shared/schema';
...
export class CognitoSyncService {
  ...
  /**
   * 🎓 UPSERT ALUNO (Baseado na implementação Python)
   * INSERT INTO alunos (usuario_id, empresa_id) VALUES (%s, %s) ON CONFLICT (usuario_id) DO NOTHING
   */
  private async _upsert_aluno(usuario_id: number, empresa_id: number): Promise<void> {
    try {
      console.log(`📝 Upsert aluno: usuario_id=${usuario_id}, empresa_id=${empresa_id}`);
      ...
      console.log(`✅ Aluno upserted: usuario_id=${usuario_id}, empresa_id=${empresa_id}`);
